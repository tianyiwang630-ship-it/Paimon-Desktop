...
class PermissionManager:
        ...
        if is_write or is_delete:
            ...
            if outside_info is not None:
                return self._record_decision(
                    ...
                    tool,
                    args,
                    "write/delete target is outside workspace",
            ...
            return self._record_decision("allow", tool, args, "write operation inside workspace")
    ...
    def can_allow_session(self, tool: str, args: Dict[str, Any]) -> bool:
        return not self._is_delete_operation(tool, args or {})
    ...
    def _is_path_outside_workspace(self, raw_path: str) -> tuple[bool, str]:
        ...
        for unresolved in _UNRESOLVED_PATTERNS:
            if unresolved.search(value):
                return True, "dynamic path expression is not allowed"

        if value.startswith("\\\\"):
            return True, "UNC path is outside workspace"
        ...
        try:
            ...
            if candidate_path.is_absolute():
                ...
                if ".." in candidate_path.parts:
                    return True, "relative parent traversal is not allowed"
                resolved = (self.workspace_root / candidate_path).resolve()

            resolved.relative_to(self.workspace_root)
            return False, ""
        except Exception:
            return True, "path escapes workspace root"
