...
const REQUEST_RESULT_SKEW_MS = 1500
const STATUS_RETRY_WEAK_NOTICE =
  'Network instability detected. Retrying (task will continue).'
...
function hasFinalAssistantSince(messages: Message[], sinceMs: number): boolean {
  const list = messages || []
  ...
  for (const message of messages || []) {
    ...
    if (ts === null) continue
    if (ts >= sinceMs - REQUEST_RESULT_SKEW_MS) {
      return true
    }
  }

  let recentUserIndex = -1
  for (let i = list.length - 1; i >= 0; i -= 1) {
    const message = list[i]
    if (message.role !== 'user') continue
    recentUserIndex = i
    const ts = parseIsoTimestamp(message.created_at)
    if (ts !== null && ts >= sinceMs - REQUEST_RESULT_SKEW_MS) {
      break
    }
  }
  if (recentUserIndex < 0) return false

  for (let i = recentUserIndex + 1; i < list.length; i += 1) {
    if (isFinalAssistantMessage(list[i])) return true
  }
  return false
}
...
function isRequestTimeoutError(error: any): boolean {
  ...
  return error?.code === 'ECONNABORTED' || message.includes('timeout of ')
}
...
export default function ChatPage() {
  ...
  const currentStatusRetryNotice = currentSessionId ? statusRetryNoticeBySession[currentSessionId] || '' : ''
  ...
  const clearSessionRuntimeState = (sessionId: string | null | undefined, abort = false) => {
    ...
    setStatusRetryNoticeBySession((prev) => {
  ...
  const clearStatusRetryNotice = (sessionId: string) => {
    setStatusRetryNoticeBySession((prev) => {
      if (!(sessionId in prev)) return prev
      const next = { ...prev }
      delete next[sessionId]
  ...
  const setStatusRetryNotice = (sessionId: string, message: string) => {
    setStatusRetryNoticeBySession((prev) => {
      if (prev[sessionId] === message) return prev
      return { ...prev, [sessionId]: message }
    })
  }

  const buildStatusRetryHandlers = (sessionId: string, epoch: number): SendMessageOptions => ({
    onStatusRetrying: (consecutiveFailures) => {
      if (!isRequestCurrent(sessionId, epoch)) return
      if (consecutiveFailures < 2) return
      setStatusRetryNotice(sessionId, STATUS_RETRY_WEAK_NOTICE)
    },
    onStatusRecovered: () => {
      if (!isRequestCurrent(sessionId, epoch)) return
  ...
  const runPendingPermissionRequest = async (sessionId: string, pendingRequestId: string) => {
    ...
    try {
      ...
      if (deferredError) {
        const latestMessages = useSessionStore.getState().currentMessages || []
        const timeoutError = isRequestTimeoutError(deferredError)
        const finalArrived = hasFinalAssistantSince(latestMessages as Message[], requestStartedAtMs)
        if (timeoutError && finalArrived) {
          // Keep timeout as silent when final response already landed.
        } else if (!timeoutError) {
          addOptimisticMessage({
            role: 'assistant',
            content: `[Error] ${deferredError?.message || 'Request failed'}`,
          })
        }
      }
      window.setTimeout(() => {
              ...
              {currentPending && (
                <div className="mb-4 text-left">
                  <div className="inline-block max-w-2xl rounded-xl bg-[var(--app-assistant-bubble)] px-4 py-3 text-[var(--app-assistant-bubble-text)]">
                    ...
                    {currentStatusRetryNotice && (
                      <div className="mt-2 text-xs text-[var(--app-text-muted)]">{currentStatusRetryNotice}</div>
                    )}
                    <details className="mt-2 rounded-xl border border-[var(--app-border)] bg-white/70 p-2 text-left text-xs">
                      <summary className="cursor-pointer text-[var(--app-text-muted)]">
