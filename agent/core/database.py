...
class Database:
    ...
    def _init_schema(self):
        with self._connect() as conn:
            ...
            CREATE TABLE IF NOT EXISTS messages (
                ...
                reasoning_json TEXT,
                protocol_flags_json TEXT,
                message_format_version INTEGER DEFAULT 2,
                created_at   TEXT NOT NULL
            );
            ...
            for statement in (
                ...
                "ALTER TABLE messages ADD COLUMN protocol_flags_json TEXT",
                "ALTER TABLE messages ADD COLUMN message_format_version INTEGER DEFAULT 2",
            ):
                try:
    ...
    def add_message(
        ...
        protocol_flags: List[str] | None = None,
        message_format_version: int | None = None,
    ) -> int:
        now = datetime.now().isoformat()
        tool_calls_json = json.dumps(tool_calls, ensure_ascii=False) if tool_calls else None
        raw_payload_json = json.dumps(raw_payload, ensure_ascii=False) if raw_payload is not None else None
        reasoning_json = json.dumps(reasoning_blocks, ensure_ascii=False) if reasoning_blocks else None
        protocol_flags_json = json.dumps(protocol_flags, ensure_ascii=False) if protocol_flags else None
        with self._connect() as conn:
            cur = conn.execute(
                """
                ...
                (
                    ...
                    reasoning_json,
                    protocol_flags_json,
                    message_format_version if message_format_version is not None else 1,
                    now,
                )
            )
    ...
    def get_messages(self, session_id: str) -> List[Dict]:
        ...
        for row in rows:
            ...
            if "provider" in row.keys() and row["provider"] is not None:
                msg["provider"] = row["provider"]

            if "raw_payload_json" in row.keys() and row["raw_payload_json"]:
                try:
                    msg["raw_payload_json"] = json.loads(row["raw_payload_json"])
                except Exception:
                    msg["raw_payload_json"] = None

            if "reasoning_json" in row.keys() and row["reasoning_json"]:
                try:
                    msg["reasoning_blocks"] = json.loads(row["reasoning_json"])
                except Exception:
                    msg["reasoning_blocks"] = []

            if "protocol_flags_json" in row.keys() and row["protocol_flags_json"]:
                try:
                    msg["protocol_flags"] = json.loads(row["protocol_flags_json"])
                except Exception:
                    msg["protocol_flags"] = []

            if "message_format_version" in row.keys() and row["message_format_version"] is not None:
                msg["message_format_version"] = int(row["message_format_version"])
            else:
                msg["message_format_version"] = 1
    ...
    def replace_messages(self, session_id: str, messages: List[Dict[str, Any]]):
        """Replace the full persisted history for one session."""
        now = datetime.now().isoformat()
        with self._connect() as conn:
            conn.execute("DELETE FROM messages WHERE session_id = ?", (session_id,))
            for msg in messages or []:
                tool_calls = msg.get("tool_calls")
                raw_payload = msg.get("raw_payload_json")
                reasoning_blocks = msg.get("reasoning_blocks")
                protocol_flags = msg.get("protocol_flags")
                conn.execute(
                    """
                    INSERT INTO messages
                        (
                    ...
                    (
                        ...
                        json.dumps(reasoning_blocks, ensure_ascii=False) if reasoning_blocks else None,
                        json.dumps(protocol_flags, ensure_ascii=False) if protocol_flags else None,
                        int(msg.get("message_format_version") or 1),
                        now,
                    ),
                )
            conn.execute(
                ...
                WHERE id = ?
                """,
                (now, len(messages or []), session_id),
            )
