...
function Invoke-NpmInDir {
    ...
    try {
        $argLine = ($Args -join " ").Trim()
        & cmd /c "npm $argLine"
        if ($LASTEXITCODE -ne 0) {
            throw "npm command failed ($LASTEXITCODE) in ${WorkingDirectory}: npm $argLine"
        }
    }
    finally {
        Pop-Location
...
function Ensure-NodeSkillDependencies {
    param(
        [Parameter(Mandatory = $true)][string]$NodeRoot,
        [Parameter(Mandatory = $true)][string]$NodeExe,
        [Parameter(Mandatory = $false)][string]$PlaywrightVersion
    )

    $npmCli = Join-Path $NodeRoot "node_modules\npm\bin\npm-cli.js"
    if (-not (Test-Path -LiteralPath $npmCli)) {
        throw "Bundled npm CLI not found in Node runtime: $npmCli"
    }

    $packageJsonPath = Join-Path $NodeRoot "package.json"
    if (-not (Test-Path -LiteralPath $packageJsonPath)) {
        $runtimePkg = [ordered]@{
        ...
        $runtimePkg | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $packageJsonPath -Encoding UTF8
    }

    $deps = @("pptxgenjs", "sharp", "react", "react-dom", "react-icons")
    if ($PlaywrightVersion) {
        $deps += "playwright@$PlaywrightVersion"
    }
    else {
        $deps += "playwright"
    }

    Write-Host "[prepare:runtimes] Install bundled Node skill dependencies: $($deps -join ', ')"

    $oldSkipBrowserDownload = $env:PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD
    try {
        $env:PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD = "1"
        & $NodeExe $npmCli install --prefix $NodeRoot --omit=dev --no-package-lock --no-audit --fund=false @deps
        if ($LASTEXITCODE -ne 0) {
            throw "Bundled Node dependency install failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        if ($null -ne $oldSkipBrowserDownload) {
...
if (Test-Path -LiteralPath (Join-Path $rednoteDir "package.json")) {
    $rednoteNodeModules = Join-Path $rednoteDir "node_modules"
    $rednoteTsc = Join-Path $rednoteNodeModules "typescript\bin\tsc"
    $rednoteDistIndex = Join-Path $rednoteDir "dist\index.js"
    $needRednoteCi = (-not (Test-Path -LiteralPath $rednoteNodeModules)) -or (-not (Test-Path -LiteralPath $rednoteTsc))

    if ($needRednoteCi) {
        Write-Host "[prepare:runtimes] rednote dependencies incomplete, run npm ci..."
        Invoke-NpmInDir -WorkingDirectory $rednoteDir -Args @("ci")
    }

    Write-Host "[prepare:runtimes] Build rednote..."
    Invoke-NpmInDir -WorkingDirectory $rednoteDir -Args @("run", "build", "--if-present")
    if (-not (Test-Path -LiteralPath $rednoteDistIndex)) {
        throw "rednote build finished but required output is missing: $rednoteDistIndex"
    }
